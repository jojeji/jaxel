...
## Corpus Check
- 12 files · ~98,593 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 781 nodes · 1137 edges · 74 communities (41 shown, 33 thin omitted)
...
Cohesion: 0.07
Nodes (29): saveFile/saveFileAs (apps/editor/src/state/document-store.ts), captureByteRanges(), clearByteRanges(), restoreByteRanges(), syncByteRangesAfterSave(), ByteRangeSnapshot, CommandBus, Command (+21 more)

### Community 1 - "npm-Dependencies (editor)"
Cohesion: 0.08
Nodes (36): App(), ToastEntry, detectFormat(), DocsState, NEW_DOCUMENT_SKELETON, nextUntitledPath(), OpenDocumentState, persistSaved() (+28 more)
...
Cohesion: 0.13
Nodes (22): Entscheidungslog — Jaxel, Arbeitsmodus: autonom bis zum ersten lauffähigen Linux-Editor (AP0-AP5), Baum-Drag&Drop-Transparenz via eigenes halbtransparentes Drag-Bild, XML↔JSON-Vereinheitlichung: eigenes DocNode-Modell statt Badgerfish, Kodierung: UTF-8/UTF-16 + Auto-Erkennung Alt-Encodings via encoding_rs, Erkennung externer Dateiänderungen + Reload nur bei Fenster-Fokus, Fokus-Ansicht: virtuelles Dokument ab Knoten X auf demselben lebenden Baum, Bekannte Einschränkung: Ein-Element-Arrays nicht von Einzelwert unterscheidbar (+14 more)
...
Cohesion: 0.17
Nodes (16): Jaxel 0.3.0 — Trefferliste als Tabelle, Base64-Decode-Ansicht: Heuristik + Kontextmenü-Fallback, read-only, Priorisierung der Desktop-Reife-Lücken (Ungespeichert-Warnung, Öffnen-mit, Sitzung wiederherstellen), Strg+F als Fokus-Shortcut (kein Toggle) + layoutstabile Toast-Meldungen, Ist-Stand — Jaxel (status.md), AP10 — Ungespeichert-Warnung beim Tab- und Fenster-Schließen, AP11 — 'Öffnen mit' bei laufender App: Weiterleitung an laufende Instanz, AP12 — Sitzung wiederherstellen (+8 more)
...
Cohesion: 0.17
Nodes (13): Jaxel 0.1.0 — Erste Veröffentlichung, Absturz- und Fehler-Logging (AP15): Frontend-Logging-Brücke, Rust nur Fehlschläge, Datenschutz-Invariante Logging: nur Pfade, Fehlermeldungen, Metadaten, Große Dateien: Streaming-Parse + Byte-Offset-Index, virtualisierte Baumansicht, Multi-Window: Tabs als Standard, echte OS-Fenster optional, AP15 — Absturz- und Fehler-Logging, AP2 — Baumansicht + Datei öffnen, AP3 — Editieren (Wert/Name/Attribute, Undo/Redo, Einfügen/Löschen) (+5 more)

### Community 21 - "CommandBus Undo/Redo/Baseline"
Cohesion: 0.21
Nodes (9): captureChangeBaseline(), ChangeBaseline, ChangeBaselineNode, ChangeSet, computeChanges(), EMPTY_CHANGES, Tombstone, findByName() (+1 more)
...
Cohesion: 0.20
Nodes (9): Jaxel 0.2.0 — Fünf Themes, portabler Windows-Build, Jaxel 0.3.1 — Suchtreffer-Wert-Spalte-Fix, Jaxel 0.3.2 — XML-Deklaration ging verloren, Jaxel 0.4.1 — Baum kollabierte beim Tab-Wechsel, Nachtrag 2026-07-19 — Fünf neue Themes (Nordlicht, Tanne, Terrakotta, Kobalt, Kontrast), Nachtrag 2026-07-20 — GitHub-Actions-Release-Workflow, Nachtrag 2026-07-22 — Suchtreffer-Tabelle: Wert-Spalte zeigt Elementinhalt statt Namen, Nachtrag 2026-07-22 — Baum kollabierte beim Tab-Wechsel (+1 more)

### Community 27 - "Status-Log AP0-AP7"
Cohesion: 0.25
Nodes (9): Unreleased (2 kritische XML-Speicher-Bugs), Jaxel 0.4.0 — Ungespeicherte Änderungen sichtbar, Byte-Offsets nach dem Speichern auffrischen (syncByteRangesAfterSave), Sichtbare Kennzeichnung ungespeicherter Änderungen: Tab-Punkt + Baum-Marker, Round-Trip: format-erhaltend, best effort, Save-Epoche: byteRange-Invalidierung im CommandBus zentralisiert, AP16 — Ungespeicherte Änderungen sichtbar machen, Nachtrag 2026-07-24 — byteRange-Invalidierung zentralisiert (Save-Epoche) (+1 more)

### Community 28 - "XML-Serialisierung (xml-export.ts)"
Cohesion: 0.25
Nodes (4): ReloadDialog(), ReloadDialogProps, Sichere Entscheidung bei externen Dateiänderungen: explizite Reload-Wahl, Nachtrag 2026-07-21 — Reload-Dialog bei externen Änderungen
...
## Knowledge Gaps
- **248 isolated node(s):** `Locale`, `catalogs`, `I18nContextValue`, `I18nContext`, `AboutDialogProps` (+243 more)
...
- **Why does `Entscheidungslog — Jaxel` connect `App.tsx Hauptkomponente` to `Status-Log AP0-AP7`, `tsconfig (core)`, `XML-Serialisierung (xml-export.ts)`, `Fehlerbehandlung & Logging`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `CommandBus` connect `Release-Historie (CHANGELOG)` to `Kickoff-Plan (AP0-AP15)`, `Status-Log AP0-AP7`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `Ist-Stand — Jaxel (status.md)` connect `Fehlerbehandlung & Logging` to `App.tsx Hauptkomponente`, `tsconfig (core)`, `Tauri Capabilities/Permissions`, `Status-Log AP0-AP7`, `XML-Serialisierung (xml-export.ts)`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
